...
font-size:12px; color:white; }} 
      ...
      .stButton>button {{ border-radius:10px; border:2px solid {HUGE_BLUE}; 
color:{HUGE_BLUE}; }} 
      .stButton>button:hover {{ filter: brightness(1.05); }} 
      div.stButton>button[kind="secondary"] {{ border:2px solid {HUGE_ORANGE}; 
color:{HUGE_ORANGE}; }} 
      div.stButton>button[kind="primary"] {{ background:{HUGE_BLUE}; color:white; border:2px 
solid {HUGE_BLUE}; }} 
...
def list_tools_by_category(cat: str) -> pd.DataFrame: 
    ...
    if not df.empty: 
        df["available_qty"] = (df["quantity"].fillna(0).astype(int) - 
df["current_out"].fillna(0).astype(int)).clip(lower=0) 
    return df 
...
def record_checkin(tool_id: int, user_name: str) -> bool: 
    db_exec("update tools set current_out = greatest(current_out - 1, 0) where id = :tid", {"tid": 
tool_id}) 
    db_exec( 
...
def read_transactions(limit: int = 100) -> pd.DataFrame: 
    ...
    sql = """ 
        select t.ts, 
               t.user_name, 
               case t.action when 'check_out' then 'Checked Out' when 'check_in' then 'Checked In' 
end as action, 
...
st.markdown(f'<div class="hh-left"><div class="hh-title">{APP_BRAND} — 
Inventory</div></div>', unsafe_allow_html=True) 
...
if active_cat in TEXT_LOGS: 
    ...
    with st.form(f"log_form_{table}"): 
        entry = st.text_input("Describe what you’re taking:", placeholder="e.g., 1 Milwaukee bag 
with 2 fine tool blades") 
        submitted = st.form_submit_button("Submit") 
        if submitted and entry.strip(): 
            log_text("extra" if table == "extra_material_log" else "bags", 
st.session_state["current_user"], entry) 
            ...
            with st.container(border=True): 
                ...
                if st.session_state["is_admin"]: 
                    with st.expander("Admin: edit / delete", expanded=False): 
                        ...
                        new_qty = st.number_input("Quantity", value=qty, min_value=0, step=1, 
key=f"qt_{tool_id}") 
                        ...
                        delete_history = st.checkbox("Also delete history", value=False, 
key=f"dh_{tool_id}") 
...
if st.session_state["is_admin"]: 
    with st.expander("Admin — Activity Log (last 100)", expanded=False): 
        ...
        if tx.empty: 
            ...
            tx = tx.rename(columns={"ts": "Timestamp", "user_name": "User", "action": "Action", 
"tool_name": "Tool"}) 
